...
def evaluate_model(model, image_model, audio_model, multi_val_loader, criterion, device):
    ...
    with torch.no_grad():
        for (val_audio_data_d, val_audio_data_r, val_image_data, val_target) in multi_val_loader:
            ...
            total_loss += multi_loss  * audio_curr_batch_size
...
def test_model(image_model, audio_model, device, path, data_dir, type, pad_sec, batch_size, audio_class):#model, image_model, audio_model, multi_test_loader, device ):
    ...
    multi_test_loader=make_loader(data_dir=data_dir, type=type, pad_sec=pad_sec, batch_size=batch_size, audio_class=audio_class)
    model.eval()
    num_correct = 0.0
    audio_num_total = 0.0
    acu_pred=[]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="save path directory (default: " ")",
        type=str,
        default='/ourdata_mfcc_two_ch_best_model/oneloss_10',
    )    
    parser.add_argument(
        "--board_dir",
        help="save tesnsor board directory (default: " ")",
        type=str,
        default='/data1/josephlee/multimodal/tensor_log/ourdata_mfcc_tensorboard_two_ch/our_oneloss_10',
    )
    parser.add_argument(
        "--seed",
...
def main():
    ...
    #                                        path=path,data_dir=args.data_dir, type='test', pad_sec=args.pad_sec, batch_size=2, audio_class=args.audio_class)
    # # 파일에 저장할 문자열 형태로 포맷팅
