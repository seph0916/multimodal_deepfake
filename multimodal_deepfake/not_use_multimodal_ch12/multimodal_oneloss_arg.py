...
def train_img_audio_model(model, image_model, audio_model, data_dir , optimizer, criterion, device, num_epochs,
    ...
    for epoch in range(num_epochs):
        ...
        for (audio_data, image_data, target) in multi_train_loader:
            ...
            # TODO: image부분에 대해서는 avceleb은 필요가 없음
            # image_loss=criterion.ce_loss(image_output, target)
            multi_loss = criterion.ce_loss(multi_output, target)

            
            # accumulate loss
            loss = multi_loss #+ audio_loss  #+ image_loss  # 하이퍼 파라미터를 좀 줌으로써 의미있는 부분에 대한 가중치를 넣어보자, 이미지가 지금 다 리얼아닌지?
            total_loss += loss
...
def evaluate_model(model, image_model, audio_model, multi_val_loader, criterion, device):
    ...
    with torch.no_grad():
        for (val_audio_data, val_image_data, val_target) in multi_val_loader:
            ...
            #image_curr_batch_size=val_image_data.size(0) TODO: 이부분은 avceleb에서는 필요가 없을듯.
            ...
            total_loss += multi_loss  #+ audio_loss # + image_loss 오디오에 대한 가중치를 좀 낮춰 봄
...
def test_model(image_model, audio_model, device, path, data_dir, type, pad_sec, batch_size, audio_class):#model, image_model, audio_model, multi_test_loader, device ):
    torch.cuda.empty_cache()
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="save path directory (default: " ")",
        type=str,
        default='/ourdata_wave_one_ch_best_model/oneloss',
    )    
    parser.add_argument(
        "--board_dir",
        help="save tesnsor board directory (default: " ")",
        type=str,
        default='/data1/josephlee/multimodal/tensor_log/ourdata_wave_tensorboard_one_ch/our_oneloss_ch1',
    )
    parser.add_argument(
        "--seed",
...
def main():
    ...
    # 파일에 저장할 문자열 형태로 포맷팅
    data_to_write = f"Test Accuracy: {test_acc}, AUC: {auc}, Recall: {recall}, F1 Score: {f1}\n use model audio {args.audio_model} image {args.image_model}\n"
