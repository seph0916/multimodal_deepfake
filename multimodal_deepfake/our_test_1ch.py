...
path="/data1/josephlee/multimodal/best_model/ourdata_mfcc_one_ch_best_model/twoloss_30"
...
multi_test_loader=make_loader(data_dir=root, type='test', pad_sec=30, batch_size=1, audio_class='mfcc')
model.eval()


test_acc, recall, f1, auc =test_model(image_model='CNN', audio_model="CNN",device= device, data_dir= multi_test_loader)
data_to_write = f"save model {path} \n Test Accuracy: {test_acc}, AUC: {auc}, Recall: {recall}, F1 Score: {f1} \n"
file_path = "/data1/josephlee/multimodal/result_ch1.txt"    
