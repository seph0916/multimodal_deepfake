...
class PadDataset(Dataset):  #TODO : vid_fps, select 변수 추가
    ...
    def __getitem__(self, index):
        ...
        if len(waveform)<=self.cut or len(images)<=self.fps * self.second:
            ...
            images = torch.tile(images, (num_img_repeats,1,1,1))
        max_point = len(waveform) - self.sample * self.second 
        start_point_wave = int(random.uniform(0,max_point))
        start_point_image = int(start_point_wave / (len(waveform) / len(images)))
        
        
        #TODO : wave파일 repeat수 체크 필요 -> elif 써서 2개 피쳐 다르게 다뤄야할듯
        waveform=waveform[start_point_wave : start_point_wave+self.cut]
        # 선택할 인덱스 계산
        indices = torch.arange(start_point_image, images.size(0), self.select)
        selected_image = images[indices]
        selected_image = selected_image[:self.second * self.fps,]
        if selected_image.size(0)<=self.second*self.fps:
            selected_image = torch.tile(selected_image,(2,1,1,1))
            selected_image = selected_image[:self.second * self.fps,]
